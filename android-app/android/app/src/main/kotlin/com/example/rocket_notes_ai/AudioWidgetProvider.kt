...
/**
 * Widget provider per la registrazione rapida di note audio
 */
class AudioWidgetProvider : AppWidgetProvider() {
    ...
    companion object {
        fun updateAppWidget(
            ...
            val layoutId = context.resources.getIdentifier("audio_widget_layout", "layout", context.packageName)
            val views = RemoteViews(context.packageName, layoutId)

            // Crea l'intent per aprire l'app con deep link
            val intent = Intent(Intent.ACTION_VIEW).apply {
                data = Uri.parse("rocketnotes://audio")
